...
def run_toys_ks(reference, data, filename, N0, N1, rng, n_toys=10, std=True, p=None, replacement=False):
    ...
    for i in toys:
        ...
        for j in range(dim):
            test, p = ks_2samp(Xnorm[Y.flatten()==0][:,j],Xnorm[Y.flatten()==1][:,j],alternative='two-sided')
            ks_tests.append(test)
            ks_tests.append(p)
        ...
        with open(filename, 'a') as f:
            f.write('{},{}\n'.format(i,','.join(str(k) for k in ks_tests)))
...
def plot_lkratio(ref,data,ref_preds,xlabel=r"$t^{(1)}_{drift}$ (ns)",label=None,save_path=None):
    ...
    ref_weights = (data.shape[0]/ref.shape[0])*np.ones_like(ref)
    learned_weights = (data.shape[0]/ref.shape[0])*np.exp(ref_preds).reshape(ref.shape)

    ref_bins, _ = np.histogram(ref,  bins=bins, weights = ref_weights)
    data_bins, _ = np.histogram(data,  bins=bins)
    learned_data_bins, _ = np.histogram(ref, bins=bins, weights = learned_weights)

    learned_ratio = (learned_data_bins+1e-10)/(ref_bins+1e-10)
    ratio = (data_bins+1e-10)/(ref_bins+1e-10)

    plt.plot(x,learned_ratio, '-o', color='lightskyblue', label=label)
    #plt.errorbar(x, ratio, yerr=np.sqrt(data_bins+1e-10)/(ref_bins+1e-10), ls='', marker='o', label ='DATA/REF', color='black')

    if np.max(learned_ratio)>3: plt.ylim(0,np.max(learned_ratio)+0.5)
    else: plt.ylim(0,3)

    plt.xlabel(xlabel,fontsize=14)
    plt.ylabel("p(x|1)/p(x|0)",fontsize=14)

    if label: plt.legend(loc='upper right',fontsize=14)
...
def plot_inputs_lkratio(ref,data,ref_preds,ref_weight=1,data_weight=1,title=None,xlabel=r"$t^{(1)}_{drift}$ (ns)",labels=('Ref','Data'),save_path=None):
    ...
    ref_weights = np.exp(ref_preds).reshape(ref.shape)

    ref_bins, _ = np.histogram(ref,  bins=bins)
    learned_data_bins, _ = np.histogram(ref, bins=bins, weights = ref_weights)
    ...
    axs[0].hist(ref, bins=bins, weights=np.ones_like(ref)*ref_weight, alpha=0.7, color='lightskyblue', edgecolor='black', label=labels[0])
    axs[0].hist(data, bins=bins, weights=np.ones_like(data)*data_weight, fill=False, edgecolor='green',lw=3.5,  label=labels[1])
    axs[0].ticklabel_format(axis='y',style='sci',scilimits=(1,2))


    axs[1].plot(x,(learned_data_bins+1e-4)/(ref_bins+1e-4), color='lightskyblue', label=labels[1])
    axs[1].ylabel(r"$\frac{p(x|1)}{p(x|0)}$",fontsize=14)
...
def plot_reco(ref,data,ref_preds,ref_weight=1,data_weight=1,title=None,xlabel=r"$t^{(1)}_{drift}$ (ns)",labels=('Ref','Data'),save_path=None):
    ...
    gs = fig.add_gridspec(ncols=1, nrows=2, hspace=0, height_ratios=[1.5, 1])
    axs = gs.subplots(sharex=True)

    axs[0].hist(ref, bins=bins, weights=np.ones_like(ref)*ref_weight, alpha=0.7, edgecolor='dimgrey', label=labels[0])
    axs[0].hist(data, bins=bins, weights=np.ones_like(data)*data_weight, histtype='stepfilled', facecolor='none', edgecolor='k', lw=3.5,  label=labels[1])

    axs[0].set_ylabel("Events",fontsize=22)
    axs[0].tick_params(labelsize=16)
    axs[0].ticklabel_format(axis='y',style='sci',scilimits=(1,2))

    axs[0].legend(loc='upper right',fontsize=18)

    if title: axs[0].set_title(title,fontsize=22)
    ...
    axs[1].plot(x,learned_ratio, '-o', label=labels[1])
    
    if np.max(learned_ratio)>2: axs[1].set_ylim(0,np.max(learned_ratio)+0.5)
    else: axs[1].set_ylim(0,2)

    axs[1].set_xlabel(xlabel,fontsize=22)
    axs[1].set_ylabel("Learned ratio",fontsize=22)

    axs[1].tick_params(labelsize=16)
    axs[1].get_yticklabels()[-1].set_visible(False)
